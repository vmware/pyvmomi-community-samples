...
set up an entire cluster with vSAN ESA, including DRS, HA, vDS, ESX networking
and core ESX services..

It takes four steps to build a HCI Cluster:
...
4.configure HCI by enabling vSAN ESA using API ConfigureHCI_Task().
...
def main():
   ...
   clusterSpec = vim.ClusterConfigSpecEx(
                  inHciWorkflow = True)
   vsanConfig = vim.vsan.cluster.ConfigInfo(enabled=True, vsanEsaEnabled=True)
   ...
   vSanCfgInfo = vim.vsan.cluster.ConfigInfo(
           enabled=True, vsanEsaEnabled=True)
