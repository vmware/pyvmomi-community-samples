...
def get_args():
    ...
    parser.add_argument('-l', '--policy', required=True,
                        ...
                        default='always',
                        choices=['always', 'never', 'onSoftPowerOff'],
                        help='The policy setting used to determine '
                             'when to perform scheduled upgrades for '
                             'a virtual machine.')
...
def main():
    ...
    if vm_obj:
        ...
        if vm_obj.config.scheduledHardwareUpgradeInfo:
            ...
            new_policy = vm_config.scheduledHardwareUpgradeInfo.upgradePolicy
            new_version = vm_config.scheduledHardwareUpgradeInfo.versionKey
            print 'Upgrade policy is now: {}'.format(new_policy)
            print 'Version key:           {}'.format(new_version)
    else:
        print "VM not found"
