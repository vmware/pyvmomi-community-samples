...
def _dictToCapability(d):
    return [
        pbm.capability.CapabilityInstance(id=pbm.capability.CapabilityMetadata.UniqueId(
                                          namespace=k.split('.')[0],
                                          id=k.split('.')[1]
        ),
...
def main():
    ...
    if vmProfile:
        ...
        print("Updating VM Storage Policy %s with %s ..." % (args.policy_name, args.policy_rule))
