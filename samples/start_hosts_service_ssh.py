    ...
    for service in services:
        if service.key == "TSM-SSH":
            if not service.running:
                print(f"Starting SSH service on {host.name}")
                service_system.StartService(service.key)
                print("SSH service started successfully.")
            else:
                print(f"SSH service is already running on {host.name}")
            return
