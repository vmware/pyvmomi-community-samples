...
url = ("https://" + vcenter_server +
       "/mob/?moid=SearchIndex&method=findByInventoryPath")

# Create global variables
global passman, authhandler, opener, req, page, page_content, nonce, headers,\
    cookie, params, e_params, syslogGhetto, clusterMoRef
...
try:
    ...
    syslog.syslog(
        syslogGhetto + ' Succesfully built HTTP Basic Authentication')

# Code to capture required page data and cookie required for post back to meet
# CSRF requirements
#
...
reg = re.compile(
    'name="vmware-session-nonce" type="hidden" value="?([^\s^"]+)"')
...
if clusterMoRef:
    syslog.syslog(
        syslogGhetto + ' Succesfully located cluster "' + cluster + '"!')
else:
    opener.close()
...
if sha1:
    syslog.syslog(
        syslogGhetto + ' Succesfully computed SHA1 hash: "' + sha1 + '"!')
else:
    opener.close()
...
xml = ('<spec xsi:type="HostConnectSpec"><hostName>%hostname</hostName>'
       '<sslThumbprint>%sha</sslThumbprint><userName>%user</userName>'
       '<password>%pass</password><force>1</force></spec>')
...
if hostip:
    syslog.syslog(
        syslogGhetto + ' Successfully extracted IP Address ' + hostip.strip())
else:
    opener.close()
...
try:
    ...
    syslog.syslog(
        syslogGhetto + ' Failed to perform DNS lookup for ' + hostip.strip())
    sys.exit(1)
else:
    syslog.syslog(
        syslogGhetto + ' Successfully performed DNS lookup for ' +
        hostip.strip() + ' is ' + host)
...
try:
    url = ("https://" + vcenter_server + "/mob/?moid=" + clusterMoRef.group()
           + "&method=addHost")
    params = {'vmware-session-nonce': nonce, 'spec': xml, 'asConnected': '1',
    ...
    url = ("https://" + vcenter_server +
           "/mob/?moid=SessionManager&method=logout")
